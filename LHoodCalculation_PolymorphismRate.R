...
StatisticRateOfPolymorphism=TRUE
...
if (StatisticRateOfPolymorphism)
{
	Pi_calculator=function(Cadaver)
	{	
		Cutoff=0.99
		AlleleFreq=Cadaver
		Temp=length(which(AlleleFreq<Cutoff & AlleleFreq>(1-Cutoff)))
		Temp=Temp[!is.na(Temp)]
		PolymorphismRate=Temp/(712)

		return(PolymorphismRate)
	}
}
...
HistMax=20
IntervalWidth=0.02
ObservedData=read.table("C:\\Users\\Dave\\Dropbox\\Chapter4\\R_Code\\PolymorphismRate.txt", sep=",", header=FALSE)
...
NucleotideDiversity=read.table("C:\\Users\\Dave\\Dropbox\\Chapter4\\R_Code\\PolymorphismRate.txt")
...
box()
axis(2, Y_AxisBreaks, cex.axis=Axis_Size, las=1)
axis(1,  c(pretty(0,1)), cex.axis=Axis_Size)
