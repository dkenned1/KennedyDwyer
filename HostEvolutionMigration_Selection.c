...
void WithinHostDrift(double TempHolder[], double InfectingStrain[], double StrainHolder[], long int Strains, int Model, gsl_rng *r, int Infectable[])
{
	long int a, b;
	double ForceOfI, Temp;

	double TempTempHolder[Strains];
	double SumTempHolder=0;
	...
	if (Model==0)
	{
		...
		{
			Temp=gsl_rng_uniform(r);			//To sync random number generator up with other models
		}
	}
	else if (Model==3)
	{
		for (a=0; a<Strains; a++)
		{
			if (Infectable[a]==1)
			{
				TempTempHolder[a]=InfectingStrain[a];
				SumTempHolder+=InfectingStrain[a];
			}
			else
			{
				TempTempHolder[a]=0;
		...
		for (a=0; a<Strains; a++)
		{
			TempHolder[a]=(TempTempHolder[a])/SumTempHolder;
			//printf("SumTempHolder=%lf\n", SumTempHolder);
		}
		for (b=0; b<StrainOverflow; b++)
		{
			Temp=gsl_rng_uniform(r);			//To sync random number generator up with other models
...
int main(int argc, char *argv[])
{
	...
	double ProbOfSusceptible=atof(argv[6]);
	...
	long int NumberOfDeaths=1e6;	//1e6;
...
//	Params.AppliedDose=844;
	...
	int **Infectable=malloc(LargeNumber*sizeof(int*));	if (Infectable==NULL){printf("Infectable too large\n");}
	for (a=0; a<LargeNumber; a++)
	{
		Infectable[a]=malloc(Initial_Strains*sizeof(int));	if (Infectable[a]==NULL){printf("Infectable too large\n");}
	}
...
//			printf("Initial_S=%ld, Initial_I=%ld, Total_V=%ld, mean_transmission=%e, j=%ld\n", Initial_S, Initial_I, TotalTotal_V, beta, j);
	...
	//printf("Here1\n");
				...
				for (b=0; b<Initial_Strains; b++)
				{
					Infectable[a][b]=gsl_ran_bernoulli(r, ProbOfSusceptible);;
					//printf("%i\n", Infectable[a][b]);
				}
			}
			...
			while(t<EndTime && (Total_E>0 || Total_I>0) && Total_S>0)				//while we're having an epidemic...
			{
				...
				else if (TypeOfNextEvent<ExposureRate)		//Next event is an exposure
				{
					...
					if (Model==3)
					{
						...
						for (b=0; b<Initial_Strains; b++)
						{
							if (I[StrainIndex][b]>EPSILON)
							{
								if (Infectable[CaterpillarIndex][b]==1)
								{
									Proceed=1;
									continue;
								}
					...
					if (DOUBLE_EQ(S[CaterpillarIndex][0], Uninfected))			//if the caterpillar was uninfected
					{
						...
						WithinHostDrift(TempHolder, I[StrainIndex], StrainHolder[WhichDeathTime], Initial_Strains, Model, r, Infectable[CaterpillarIndex]);
						for (a=0; a<Initial_Strains; a++)
						{
