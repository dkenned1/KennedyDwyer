...
StatisticNucleotideDiversity=FALSE
StatisticEffectiveAlleles=FALSE
StatisticRateOfPolymorphism=FALSE
StatisticRelativeDiversity=TRUE

if (StatisticNucleotideDiversity)
{
...
if (StatisticEffectiveAlleles)
{
	...
	{	
		...
		EffectiveAlleles=1/(1-LociDiversity)

		return(mean(EffectiveAlleles))
	}
}
if (StatisticRelativeDiversity)
{
	Pi_calculator=function(Cadaver)
	{	
		...
		return(NucleotideDiversity/PopWideHaplotypePi)
	}
	
	Pi_calculator_Consensus=function(Allele)
	{	
		Diversity=1-mean(Allele==1, na.rm=TRUE)^2-mean(Allele==0, na.rm=TRUE)^2
		return(Diversity)
	}
...
HistMax=20
IntervalWidth=0.3
ObservedData=read.table("C:\\Users\\Dave\\Dropbox\\Chapter4\\R_Code\\RelativeDiversity.txt", sep=",", header=FALSE)
ObservedData=ObservedData[,1]
#ObservedData=1/(1-ObservedData)
...
#######		TempData[which(TempData<.5)]=abs(TempData[which(TempData<.5)]-1)
		...
		SimulatedGenotypeMatrix=matrix(,143, SegregatingSites)
		for (aa in 1:143)
		{
			SimulatedGenotypeMatrix[aa,]= (TempData[aa,]>0.5)
		}
		
		PopWideHaplotypePi=mean(apply(SimulatedGenotypeMatrix, 2, Pi_calculator_Consensus))
...
#		HistStor=hist(PopDiversity, freq=TRUE, breaks=seq(LowerBound,UpperBound,IntervalWidth), xlim=X_Lim, ylim=Y_Lim, xlab="", main="")	
		HistStor=hist(PopDiversity, freq=TRUE, breaks=seq(LowerBound,UpperBound,IntervalWidth), xlim=X_Lim, xlab="", ylab="", main=zzz)	
...
NucleotideDiversity=read.table("C:\\Users\\Dave\\Dropbox\\Chapter4\\R_Code\\RelativeDiversity.txt")
NucleotideDiversity=NucleotideDiversity[,1]

#NucleotideDiversity=1/(1-NucleotideDiversity)
...
plot(X_axis, Bounds1[1,]  , xlim=c(X_Lower,X_Upper), ylim=c(0,Y_Lim), type="l", col=0, cex.axis=Axis_Size, axes=FALSE)
box()
...
plot(X_axis, Bounds2[1,]  , xlim=c(X_Lower,X_Upper), ylim=c(0,Y_Lim), type="l", col=0, cex.axis=Axis_Size, axes=FALSE)
box()
axis(2, Y_AxisBreaks, cex.axis=Axis_Size, las=1)
...
plot(X_axis, Bounds3[1,]  , xlim=c(X_Lower,X_Upper), ylim=c(0,Y_Lim), type="l", col=0, cex.axis=Axis_Size, axes=FALSE)
box()
axis(2, Y_AxisBreaks, cex.axis=Axis_Size, las=1)
axis(1,  c(pretty(1,2)), cex.axis=Axis_Size)
...
MeanLHoodModel2=log(mean(exp(LHoodStor[(0:68 * 3)+2])))
